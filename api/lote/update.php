...
if (empty($id)) {
    http_response_code(400);
    echo json_encode(["mensagem" => "ID do lote eh obrigatorio para atualizacao."]);
    exit;
}
...
try {
    $linhasAfetadas = $loteModel->atualizar($id, $dadosLote);
    ...
    if ($linhasAfetadas > 0) {
        ...
        echo json_encode(["mensagem" => "Lote nao encontrado ou nenhum dado alterado."]);
    }
} catch (Exception $e) {
